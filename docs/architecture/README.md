...
O Nginx entrega o frontend e encaminha `/api/*` ao backend. Nenhum serviço interno publica porta no host. A API registra uma execução no SQLite e envia seu identificador ao Redis. O worker consome a fila, executa o módulo, registra progresso e salva resultados mascarados no mesmo volume SQLite.
